...
with st.expander("📊 Supplement: Comparison of risk classification across kits and methods"):
    st.markdown("""
    Enter measured Lp(a) values (mg/dL) for each kit.  
    The table shows risk classification for each kit using IFCC conversion and 2.2x conversion.  
    **⚠️ icon** indicates that the 2.2x method underestimates the risk compared to IFCC conversion.
    """)
    ...
    for kit, val in user_inputs.items():
        ...
        if val > 0:
            ...
            row = {
                "Kit": kit,
                "Input (mg/dL)": val,
                "IFCC (nmol/L)": round(ifcc, 2),
                "IFCC Risk": risk_table[idx_ifcc]["class"],
                "2.2x (nmol/L)": round(x22, 2),
                "2.2x Risk": risk_table[idx_x22]["class"],
                "Caution": "⚠️" if idx_x22 < idx_ifcc else ""
            }
            data.append(row)
    ...
    if data:
        ...
        def color_row(row):
            color_ifcc = f'background-color: {risk_table[classify_lpa_risk(row["IFCC (nmol/L)"])]["color"]}'
            color_x22  = f'background-color: {risk_table[classify_lpa_risk(row["2.2x (nmol/L)"])]["color"]}'
            return [
                "", "", "", color_ifcc, "", color_x22, ""
            ]
        st.markdown("#### Risk classification comparison table")
        st.dataframe(
            df.style.apply(color_row, axis=1),
            use_container_width=True
        )
        st.caption("⚠️ : 2.2x method underestimates risk compared to IFCC conversion.")
    else:
        st.info("Enter values for at least one kit to view comparison.")
